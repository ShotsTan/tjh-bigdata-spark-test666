...
//    rdd3.persist(StorageLevel.MEMORY_ONLY)
    ...
    //    val rdd11 = sc.makeRDD(List("Hello Spark", "Hello Fink"), 2)
    //    val rdd21: RDD[String] = rdd11.flatMap(x => x.split(" "))
    //    val rdd31: RDD[(String, Int)] = rdd21.map(x => (x, 1))
    val rdd41: RDD[(String, Iterable[Int])] = rdd3.groupByKey()
    rdd41.saveAsTextFile("output11")
  }
